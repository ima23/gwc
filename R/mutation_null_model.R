...
#mut_rates <- read.delim("../data/mutation_rate_table_DNM_direct.txt")
...
p_base <- function(f){
  ...
  p = mut_rates[mut_rates$from == f, 'mu_snp']
  return(sum(p))
}

p_from_to <- function(f, t){

  # probability of mutation from base in position 2 to any of three other possible bases

  p = mut_rates[(mut_rates$from == f) & (mut_rates$to == t), 'mu_snp']
  return(p)
}
...
p_sequence_meth <- function(sequence, cpg_pos, prop_meth, correction_model) {
  ...
  cpgs_ref = str_sub(sequence, cpg_pos, cpg_pos + 2)  # because the sequence is taken from position -1 to position + 1 in order to capture all triplets
  cpgs_alt = cpgs_ref
